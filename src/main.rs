...
fn player_input(gs: &mut State, ctx: &mut Rltk) {
    ...
    if ctx.left_click {
        ...
        if map.cells[index].tile == TileType::Wall {
            map.cells[index].mark_to_dig = !map.cells[index].mark_to_dig;
        }
    }
}
    ...
    for cell in map.cells.iter() {
        match cell.tile {
            ...
            TileType::Wall if cell.mark_to_dig => {
                ctx.set(x, y, RGB::from_f32(0.2, 0.7, 0.0), RGB::from_f32(0., 0., 0.), rltk::to_cp437('#'));
            }
            TileType::Wall => {
...
fn main() -> rltk::BError {
    ...
    gs.ecs
        .create_entity()
        .with(Position { x: MAP_W - 2, y: MAP_H / 2 })
        .with(Renderable {
            glyph: rltk::to_cp437('@'),
